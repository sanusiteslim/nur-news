...
import { isElement, tagName, PRESERVE_WHITESPACE_TAGS, hasMonospaceFontFamily, hasPreservedWhitespaceStyle, HTML_BLOCK_TAGS, HTML_HEADER_TAGS, DEFAULT_SPAN, DEFAULT_BLOCK, BLOCK_DEFAULT_STYLE, HTML_LIST_CONTAINER_TAGS, HTML_SPAN_TAGS, HTML_LIST_ITEM_TAGS, HTML_DECORATOR_TAGS, isMinimalSpan, isEqualMarks, defaultParseHtml, flattenNestedBlocks, ensureRootIsBlocks, resolveJsType, isPlaceholderDecorator, isPlaceholderAnnotation, isMinimalBlock, isNodeList } from "./_chunks-es/helpers.js";
...
function preprocessWhitespace(_, doc) {
  function processNode(node) {
    ...
    !(node.parentElement && hasMonospaceFontFamily(node.parentElement) && hasPreservedWhitespaceStyle(node.parentElement))) {
...
function findAdjacentMonospaceParagraph(el, direction) {
  ...
  for (; sibling && isBlankParagraph(sibling); )
  ...
  return [
    {
      ...
      deserialize(el, _next, createBlock) {
        if (!isElement(el) || !isGoogleDocs(el) || !isRootNode(el) || !isMonospaceParagraph(el) || !schemaCanHoldCode)
          return;
        ...
        const codeObject = codeMatcher?.({
          context,
          props: { language: void 0, code: lines.join(`
`) }
        });
        if (codeObject !== void 0)
    ...
    },
    {
      ...
      deserialize(el) {
        if (!(!isElement(el) || !isGoogleDocs(el) || !isRootNode(el) || !isBlankParagraph(el) || !schemaCanHoldCode))
          return findAdjacentMonospaceParagraph(el, "previous") && findAdjacentMonospaceParagraph(el, "next") ? [] : void 0;
      }
    },
    {
      deserialize(el, next) {
    ...
    {
      deserialize(el) {
        if (tagName(el) === "br" && isGoogleDocs(el) && isElement(el) && // The class is `Apple-interchange-newline`; compare
        // case-insensitively since `classList.contains` is case-sensitive
        Array.from(el.classList).some(
          (className) => className.toLowerCase() === "apple-interchange-newline"
        ))
          return [];
    ...
    whitespaceTextNodeRule,
    {
      ...
      deserialize(el, _next, createBlock) {
        ...
        const text = el.textContent || "", codeObject = options.matchers?.code?.({
          ...
          props: { language: void 0, code: text.trim() }
        });
        if (codeObject !== void 0)
          return createBlock(codeObject);
