...
import { base, jsonResponse, jsonRequest, httpErrors, retry, promise } from "get-it/middleware";
...
const DEFAULT_API_VERSION = "v2025-06-01", DEFAULT_MAX_SYNC_ITERATIONS = 5, sharedConverter = new DescriptorConverter();
function mergeHeaders(token, headers) {
  const hasAuthHeader = headers ? Object.keys(headers).some((key) => key.toLowerCase() === "authorization") : !1;
  return {
...
function resolveRequester(options) {
  if (options.requester !== void 0 && options.baseUrl !== void 0)
    throw new Error("uploadSchema: provide either `requester` or `baseUrl`, not both");
  if (options.requester !== void 0)
...
async function prepareSchemaUpload(schema, options) {
  const requester = resolveRequester(options), apiVersion = options.apiVersion ?? DEFAULT_API_VERSION, maxSyncIterations = options.maxSyncIterations ?? DEFAULT_MAX_SYNC_ITERATIONS;
  ...
  const { contextKey } = options, permanent = options.claim === "permanent", headers = mergeHeaders(options.token, options.headers);
  function request(opts) {
    return requester({ ...opts, headers });
  }
  function url(path) {
    return `/${apiVersion}/descriptors/${path}`;
  }
  const emit = makeEmit(options.onPhaseComplete), startedConvert = performance.now(), sync = await sharedConverter.get(schema), descriptorId = sync.set.id;
  emit("convert", startedConvert);
  const startedClaim = performance.now(), claimResponse = await request({
    url: url("claim"),
    method: "POST",
  ...
  let commitId;
  if (permanent && (commitId = claimResponse.commitId, !commitId))
  ...
  for (let i = 0; i <= maxSyncIterations; i++) {
    ...
    if (syncRequest === null)
      return { descriptorId, commit: permanent ? async () => {
...
async function uploadSchema(schema, options) {
  const startedTotal = performance.now(), emit = makeEmit(options.onPhaseComplete), { descriptorId, commit } = await prepareSchemaUpload(schema, { ...options, claim: "permanent" });
...
function extractSchema(schemaDef, extractOptions = {}) {
  ...
  repeated.forEach((key, objectField) => {
    const base2 = convertSchemaType(objectField.type);
    if (base2 !== null) {
      if (base2.type === "inline") {
        repeated.delete(objectField);
        return;
      }
      if (base2.type === "unknown") {
        repeated.delete(objectField);
        return;
      }
      schema.push({
    ...
    if (schemaType === void 0)
      return;
    const base2 = convertBaseType(schemaType);
    base2 !== null && (base2.type === "type" && inlineFields.add(schemaType), base2.type === "document" && documentTypes.set(typeName, base2), schema.push(base2));
  });
