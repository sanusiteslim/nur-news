...
export async function blueprintInitCore(options) {
    ...
    const blueprintDir = isHere ? '.' : explicitDir;
    const userProvidedDirName = isHere ? undefined : explicitDir;
    ...
    try {
        ...
        if (!explicitDir && isInteractive) {
            const proceed = await confirm({
                message: `Initialize a Blueprint here in ${cwd()}?`,
                default: true,
            });
            if (!proceed) {
                log(`Run \`${bin} blueprints init <directory>\` to initialize in a different location.`);
