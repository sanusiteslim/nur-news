...
export class DoctorCommand extends SanityCommand {
    ...
    static description = 'Run diagnostics on your Sanity project';
    static examples = [
    ...
    printCheck(check) {
        ...
        const title = check.status === 'passed' ? check.title : styleText(check.status === 'error' ? 'red' : 'yellow', check.title);
    ...
    printMessage(message) {
        ...
        const text = message.type === 'error' ? styleText('red', message.text) : message.type === 'warning' ? styleText('yellow', message.text) : message.text;
    ...
    printSummary(summary) {
        ...
        if (summary.warnings > 0) {
            parts.push(styleText('yellow', `${summary.warnings} warning${summary.warnings === 1 ? '' : 's'}`));
        }
        if (summary.errors > 0) {
...
function getChecks(argv) {
    ...
    const stringArgs = argv.filter((item)=>typeof item === 'string' && !item.startsWith('-'));
    const unknownChecks = stringArgs.filter((item)=>!KNOWN_CHECKS.includes(item));
    if (unknownChecks.length > 0) {
        throw new Error(`Unknown check${unknownChecks.length === 1 ? '' : 's'}: ${unknownChecks.join(', ')}. Valid checks: ${KNOWN_CHECKS.join(', ')}`);
    }
    const checkNames = stringArgs;
    return checkNames.length > 0 ? checkNames.map((check)=>doctorChecks[check]) : Object.values(doctorChecks);
}
